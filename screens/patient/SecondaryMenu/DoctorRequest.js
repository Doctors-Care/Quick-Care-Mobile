...
function DoctorRequest({ navigation, route }) {
    ...
    const createDoctorrequest = () => {
        ...
        console.log(Request)
        axios.post("http://192.168.11.229:3000/request/addingRequest", Request).then((result) => { setidrequest(result.data.id);navigation.navigate("DoctorLoadingScreen", { requestid: idrequest }) }).catch((error) => console.log(error))
    }
    return (
        <ScrollView>
